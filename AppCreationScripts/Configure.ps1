...
Function ConfigureApplications
{
   ...
   Write-Host "Creating the AAD application (msal-next-webapp)"
   # Get a 6 months application key for the service Application
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInMonths 6
   
   # create the application 
   $serviceAadApplication = New-MgApplication -DisplayName "msal-next-webapp" `
                                                       -Web `
                                                       @{ `
                                                           RedirectUris = "http://localhost:3000/api/auth/redirect"; `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/msal-next-webapp")
    ...
    Write-Host "Done creating the service application (msal-next-webapp)"
    ...
    Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>msal-next-webapp</a></td></tr>" -Path createdApps.html
    ...
    Write-Host "Successfully registered and configured that app registration for 'msal-next-webapp' at `n $servicePortalUrl" -ForegroundColor Green 
