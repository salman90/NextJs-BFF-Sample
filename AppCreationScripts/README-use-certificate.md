...
3. Run the [Configure-withCertCertificates.ps1](./Configure-withCertCertificates.ps1) script to re-create the App Registration. The script will also create `.pfx` file(s) (e.g. msal-next-webapp.pfx) that you can upload to Key Vault later. When asked about a password, do remember it - you will need the password when uploading the certificate.
...
```bash
openssl req -x509 -newkey rsa:2048 -keyout msal-next-webapp.key -out msal-next-webapp.cer -subj "/CN=msal-next-webapp" -nodes

Generating a RSA private key
.........................................................
writing new private key to 'msal-next-webapp.key'
```

The following files should be generated: *msal-next-webapp.key*, *msal-next-webapp.cer*

If you need, you can generate a msal-next-webapp.pfx (certificate + private key combination) with the command below:

```bash
openssl pkcs12 -export -out CertificateName.pfx -inkey msal-next-webapp.key -in msal-next-webapp.cer
```

Enter an export password when prompted and make a note of it. The following file should be generated: *msal-next-webapp.pfx*.
...
>
> ```console
> cat msal-next-webapp.crt msal-next-webapp.key > msal-next-webapp.pem ## if powershell: Get-Content msal-next-webapp.crt, msal-next-webapp.key | Set-Content msal-next-webapp.pem
> openssl pkcs12 -in msal-next-webapp.pfx -out msal-next-webapp.pem
> ```
...
1. Click on **Upload** certificate and select the certificate file to upload (e.g. *msal-next-webapp*).
...
> Perform the steps below for the service app (msal-next-webapp)

1. Open the file where MSAL is initialized (e.g. `6-AdvancedScenarios\1-call-api-bff\App\app.js`).
